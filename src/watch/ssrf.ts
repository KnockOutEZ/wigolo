...
function isPrivateIpv6(host: string): boolean {
  ...
  // `2002:` (2002:7f00:1:: -> 7f00:0001 -> 127.0.0.1). ALL of 2002::/16 is 6to4, so
  // any 2002:-prefixed address decodes; a private/metadata embedding on a host with
  // 6to4 routing reaches the embedded v4, so block it. The low hextet is OPTIONAL:
  // when the embedded v4 is x.y.0.0 the trailing zero compresses away
  // (2002:7f00:0:: normalizes to [2002:7f00::]), so a missing low hextet defaults to 0.
  const sixToFour = h.match(/^2002:([0-9a-f]{1,4})(?::([0-9a-f]{1,4}))?/);
  if (sixToFour) {
    const dotted = hexPairToDotted(sixToFour[1], sixToFour[2] ?? '0');
    if (dotted && isLoopbackIpv4(dotted)) return true;
  }

  // NAT64 (64:ff9b::/96): the IPv4 is embedded in the low 32 bits — either a
  // trailing dotted quad or the last two hextets (64:ff9b::a9fe:a9fe -> 169.254.169.254).
  const nat64Dotted = h.match(/^64:ff9b::(?:.*:)?(\d+\.\d+\.\d+\.\d+)$/);
  if (nat64Dotted && isLoopbackIpv4(nat64Dotted[1])) return true;

  const nat64Hex = h.match(/^64:ff9b::(?:[0-9a-f:]*:)?([0-9a-f]{1,4}):([0-9a-f]{1,4})$/);
  if (nat64Hex) {
    const dotted = hexPairToDotted(nat64Hex[1], nat64Hex[2]);
    if (dotted && isLoopbackIpv4(dotted)) return true;
  }

  // Exotic all-zero-high-hextet forms (2002::1, 64:ff9b::1) embed 0.0.0.x and are not
  // decoded here — no realistic private/metadata target has a zero high hextet
  // (7f00/0a00/c0a8/a9fe/ac1x are all non-zero).
  return false;
}

/** Decode two IPv6 hextets into the dotted IPv4 they embed; null on unparseable input. */
function hexPairToDotted(highHex: string, lowHex: string): string | null {
