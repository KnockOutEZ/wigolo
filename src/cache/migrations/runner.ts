...
// SD4 §4.1 (A-15-4) — the annotation layer's store half: durable, per-URL-pattern, revise-chained.
// Full rationale (why per pattern, why append-plus-status, why no CHECK on `kind`) is in the
// mirrored 023-studio-annotations.sql, which a test holds byte-identical to this constant.
const MIGRATION_023_STUDIO_ANNOTATIONS = `
...
// SD6 §3 (A-17-3) — the site-profile store's schema half: three tables, no domain code. Full
// rationale (why a junction and not a second home, why no CHECK on `visibility`, why the profile
// table takes no wire cursor, why no column can carry a credential) is in the mirrored
// 024-studio-site-profiles.sql, which a test holds byte-identical to this constant.
const MIGRATION_024_STUDIO_SITE_PROFILES = `
...
// SD7 A-18-5 — the visits store: history-with-content, agent-partitioned by construction.
// Full rationale (why two tables, why no FKs, why its own FTS index) is in the mirrored
// 021-studio-visits.sql, which a test holds byte-identical to this constant.
const MIGRATION_021_STUDIO_VISITS = `
