...
export const ALLOWED_RAW: RawValueAllowance[] = [
  ...
  { path: 'search:$.images[].url', why: 'Aggregated image locator; fenceSearchData fences only the alt prose on this item.', producer: 'src/server/content-fence.ts:463', shape: 'absolute-http-url' },
  { path: 'search:$.images[].source_url', why: 'The page hosting the image, and the origin attributed to the fenced alt.', producer: 'src/server/content-fence.ts:463', shape: 'absolute-http-url' },
  { path: 'search:$.images[].thumbnail_url', why: 'Smaller preview image locator on the aggregated image inventory; the alt prose on the same item is fenced.', producer: 'src/server/content-fence.ts:463', shape: 'absolute-http-url' },
