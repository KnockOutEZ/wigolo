...
/**
 * PIN 8 (#57)'s rule, kept after its subject moved. A tool description is a budget, not a
 * scratchpad — announcing a field is supposed to cost a trim first. The two reshaped studio
 * descriptions those arms held to the cap left core with the surface, so the cap is asserted where
 * it still binds: every description core serves, with no exception for the long ones.
 */
describe('every description stays inside the per-tool budget', () => {
  const TOOL_DESC_BUDGET = 400;

  it.each(Object.keys(TOOL_DESCRIPTIONS) as ToolName[])('%s is under the per-tool token budget', (name) => {
