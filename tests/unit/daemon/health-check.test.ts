...
function makeInput(overrides?: Partial<HealthProbeInput>): HealthProbeInput {
  return {
    ...
    // Default to a LIVE cache so tests about the sidecar / browser pool aren't
    // implicitly asserting a dead cache. Cache-specific cases override this
    // explicitly (including `null`, which means "not initialized").
    cacheProbe: () => true,
    ...overrides,
  };
}
