...
describe('SKILL.md — v3 structure and content', () => {
  ...
  it('frontmatter lists all 11 tools', () => {
    content = readFileSync(SKILL_PATH, 'utf-8');
    frontmatter = parseYamlFrontmatter(content);
    expect(frontmatter).not.toBeNull();
  ...
  it('frontmatter tools include v3 additions: find_similar, research, agent, diff, watch, index', () => {
    content = readFileSync(SKILL_PATH, 'utf-8');
    frontmatter = parseYamlFrontmatter(content);
    const tools = frontmatter!.tools as Array<Record<string, unknown>>;
    const toolNames = tools.map(t => t.name);
    for (const name of ['find_similar', 'research', 'agent', 'diff', 'watch', 'index']) {
...
describe('SKILL.md — CRLF tolerance (Windows checkout)', () => {
  ...
  it('parseYamlFrontmatter returns all 11 tools from CRLF input', () => {
    const lf = readFileSync(SKILL_PATH, 'utf-8');
    const crlf = asCrlf(lf);
    const frontmatter = parseYamlFrontmatter(crlf);
    expect(frontmatter).not.toBeNull();
  ...
  it('parseYamlFrontmatter tool names from CRLF input cover all 11 tools', () => {
    const lf = readFileSync(SKILL_PATH, 'utf-8');
    const crlf = asCrlf(lf);
    const frontmatter = parseYamlFrontmatter(crlf);
    const tools = frontmatter!.tools as Array<Record<string, unknown>>;
    const toolNames = tools.map(t => t.name);
    for (const name of ['fetch', 'search', 'crawl', 'cache', 'extract', 'find_similar', 'research', 'agent', 'diff', 'watch', 'index']) {
      expect(toolNames).toContain(name);
    }
  });
