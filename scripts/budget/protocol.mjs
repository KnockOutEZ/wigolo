...
export const GATES = {
  ...
  'G-ACQUIRE': {
    ...
    limit: 880,
    baseline:
      "804 MiB measured 2026-08-11 on the GitHub macos-latest runner, on the corrected window (browsers 548 = chromium 347 + headless shell 198 + ffmpeg 3 + links 1; data 256 = fastembed 143 + transformers 98 + driver 17). A second window-correct run on the same branch read 788 (browsers 541 = chromium 341 + headless shell 198 + ffmpeg 3 + links 1; data 247 = fastembed 133 + transformers 98 + driver 17); 804 is the anchor because it is the worse of the two. ⚠ RE-DERIVED FROM 800 BECAUSE 800 REDS A CLEAN BUILD. 800 was set from a single 764 reading; the distribution behind it was never looked at. Every `clean-machine-smoke (macos-latest, node 22)` log since 2026-08-01 was harvested — 29 runs with a reading, of which 10 are the current tree — and on the current tree the gate reads 769/781/783/784/785/788/792/793/793/799, plus one red at 803 and this slice's window-correct 804. A limit of 800 sits INSIDE that distribution, which is why it failed once at 803 and passed on re-run at 769 and 788. A gate that reds randomly reds CI randomly and teaches people to re-run it. ⚠ WHAT THE DRIFT IS. It is THREE things, and the child-level breakdown added by this slice is what separates them — before it, every observation was one number per directory, which can show that something moved and can never say what. (1) LIVE-WEB CONTAMINATION, FIXED: the diff used to `du` live at assertion time, three steps and ~3 minutes after warmup exited, with a live fetch and a live search writing cached web content into the DEFAULT data directory in between — 1-13 MiB charged to `warmup`. A second snapshot taken the instant warmup exits closes that. (2) A FIXED RUNNER-VERSUS-LAPTOP OFFSET, not a regression: two window-correct runner runs report chromium-headless-shell at 198 and transformers at 98 where the same pinned content measures 191 and 88 on darwin-arm64. It is identical across runs, so it is a property of the host doing the measuring rather than of the download, and it is why the threshold is anchored to the runner. (3) A RUN-TO-RUN VARYING PART CONFINED TO TWO CHILDREN, still unexplained: across those same two runs chromium reads 347 then 341 and fastembed reads 143 then 133, while headless shell, ffmpeg, the driver and transformers are byte-identical. That siblings hold still while these two move is what rules out a global measurement artifact for this part — 16 of the 35 MiB spread is in the download. The browser side is version-pinned (`playwright@1.60.0` exact, chromium revision 1223), reproduces at 534 MiB / 358 files across two independent local installs, gains 0 bytes from a real browser launch, and never accumulates (`browsers=0MiB` at snapshot in all 29 runs) — so the chromium term should not vary at all and does. The embedding model is fetched from a remote model host at an UNPINNED revision, which is the obvious suspect for the fastembed term and the first thing the next slice should look at. Left unexplained rather than papered over, and now measured per child on every run. ⚠ THE NUMBER: 880 sits 76 MiB above the worst clean observation ever recorded on this artifact (804), which is ~2.2x its 35 MiB spread, and 158 below the smallest regression it is chartered to catch — a second browser engine acquired by warmup lands a best-case host at 766 + 272 = 1038. It therefore DETECTS a regression of roughly 114 MiB and up (880 - 766) and is BLIND BELOW THAT; an 88 MiB model duplication would pass, and no tighter number is honest while the pinned engine reads 26 MiB differently across hosts. ⚠ IT STAYS BLOCKING, and the arithmetic is why: the window between 'above every clean observation' (804) and 'below the smallest chartered regression' (1038) is 234 MiB against a statistic whose own spread is 35 — 6.7x coarser than its noise. That is the opposite of G-RSS-SUBSTRATE, whose 19 MiB window against a 21 MiB spread is what makes it report-only. ⚠ S10-d's replacement pair CANNOT be stated over this artifact — see SUBSTRATE_ONLY_ACQUISITION. ⚠ KEPT over the full artifact: it is the gate that prices the tier-INDEPENDENT models, and it is what catches amended-D1's doubling regression — a run that acquires the desktop component AND the browser engine lands at 300 + 546 + 218 = 1064 against this 880.",
  },
