...
/**
 * `node --experimental-sea-config` — the blob, which is platform-agnostic across ARCHES but NOT
 * across Node VERSIONS.
 *
 * THE INTERPRETER IS THE PINNED ONE, NEVER `process.execPath`, AND THIS COST A CI MATRIX TO FIND.
 * The blob carries a format the loader in the embedded runtime has to recognise. Generated by node
 * v22.23.2 (what `setup-node: 22` installs today) and injected into the pinned v22.14.0, every one
 * of the five artifacts built, sealed, signed and packed cleanly — and then aborted on its own
 * platform at startup with `FATAL ERROR: v8::ToLocalChecked Empty MaybeLocal` inside
 * `node::sea::LoadSingleExecutableApplication`, before a line of wigolo ran. Nothing upstream of
 * the load noticed: `--version` was the FIRST thing to fail.
 *
 * It passed on the dev host for one reason only — that host's `node -v` happened to be 22.14.0.
 * So the fix is not "pin the CI runner to 22.14.0", which would leave the trap armed for the next
 * person on a different machine; it is to stop asking the host's interpreter at all. The blob is
 * written by the same checksum-verified official binary the artifact embeds, and the version is
 * asserted rather than assumed.
 */
export function buildBlob({ workDir, bundlePath, manifest, interpreter }) {
  ...
  const reported = execFileSync(interpreter, ['-v'], { encoding: 'utf8' }).trim();
  if (reported !== `v${manifest.runtime.version}`) {
    throw new Error(
      `REFUSED — the blob interpreter reports ${reported}, runtime.json pins v${manifest.runtime.version}.\n` +
        '  A blob written by one Node version and injected into another produces an artifact that\n' +
        '  builds, seals and signs cleanly and then aborts before its first line of JavaScript.'
    );
  }

  execFileSync(interpreter, ['--experimental-sea-config', path.basename(config)], {
    cwd: workDir,
    stdio: ['ignore', 'inherit', 'inherit'],
  });
  log(`  blob     ${path.basename(blob)} (${mb(fs.statSync(blob).size)}) written by ${reported}, runtime ABI ${manifest.runtime.abi}`);
  return blob;
}
...
export async function buildTarget({ target, outDir, manifest = readManifest(), keepStage = false }) {
  ...
  // The blob's interpreter is the pinned runtime for the BUILD HOST — a second download when the
  // host is not the target, and the same cache, because the alternative is the failure mode
  // documented on `buildBlob`. A host platform the pin file has no digest for cannot write a blob
  // this pipeline is able to vouch for, so it is a refusal and not a fallback to `process.execPath`.
  const hostTarget = `${process.platform}-${process.arch}`;
  if (!manifest.runtimeTarballSha256?.[hostTarget]) {
    throw new Error(
      `REFUSED — this build host is ${hostTarget}, which runtime.json has no pinned runtime for.\n` +
        '  The SEA blob must be written by the exact Node version the artifact embeds (see buildBlob).'
    );
  }
  const hostRuntime =
    hostTarget === target ? runtime : await fetchRuntime({ manifest, target: hostTarget });

  const { outfile: bundlePath, metafile } = bundle({ workDir, manifest });
  assertBundleImports(metafile);
  const blob = buildBlob({ workDir, bundlePath, manifest, interpreter: hostRuntime.binPath });
